...
var CardControler = {
	...
	//根据牌的数组，计算出出现了几种牌值（A-Joker），存放到aCardsVal中，
	//将每种牌对应的牌号（1-54），根据牌值，存放到oCardsVal中。
	...
	//牌型表和对应检查方法
	oCardsTypeList: {
		'fJudge_DAN': CARDSTYPE.YI_ZHANG,
		'fJudge_SHUANG': CARDSTYPE.ER_ZHANG,
		'fJudge_SAN': CARDSTYPE.SAN_ZHANG,
		'fJudge_ZHA': CARDSTYPE.SI_ZHANG,
		'fJudge_DAN_SHUN': CARDSTYPE.YI_SHUN,
		'fJudge_SHUANG_SHUN': CARDSTYPE.ER_SHUN,
		'fJudge_SAN_SHUN': CARDSTYPE.SAN_SHUN,
		'fJudge_WANG_ZHA': CARDSTYPE.HUO_JIAN,
		'fJudge_FEI_JI': CARDSTYPE.FEI_JI,
		'fJudge_SAN_DAI_YI': CARDSTYPE.SAN_DAI_YI,
		'fJudge_SI_DAI_ER': CARDSTYPE.SI_DAI_ER,
	},

	fGetCardsType: function(aCards) {
		var self = this;
		var oCheckList = this.oCardsTypeList;
		var type = 0;
		var oRet = self.fGetCardsNumObj(aCards);
		var oCardsNum = oRet['oCardsNum'];
	...
	fJudge_DAN: function(nLen, oCardsNum) {
		var ret = nLen == 1 ? true : false;
		return ret;
	},
	fJudge_SHUANG: function(nLen, oCardsNum) {
		var ret = (nLen == 2) && (oCardsNum['2'].length == 1) ? true : false;
		return ret;
	},
	fJudge_SAN: function(nLen, oCardsNum) {
		var ret = (nLen == 3) && (oCardsNum['3'].length == 1) ? true : false;
		return ret;
	},
	fJudge_ZHA: function(nLen, oCardsNum) {
	...
	fJudge_DAN_SHUN: function(nLen, oCardsNum) {
		...
		var aTemCardsVal = oCardsNum['1'].sort(self.SORTASC);
		for (var i = 1; i < aTemCardsVal.length; i++) {
			var nForeCard = i - 1;
			var nVardVal = aTemCardsVal[i];
			var nForeCardVal = aTemCardsVal[nForeCard];
			if (nVardVal - nForeCardVal != 1) {
				return false;
			}
		}
		return true;
	...
	fJudge_SHUANG_SHUN: function(nLen, oCardsNum) {
		...
		var aTemCardsVal = oCardsNum['2'].sort(self.SORTASC);
		for (var i = 1; i < aTemCardsVal.length; i++) {
			var nForeCard = i - 1;
			var nVardVal = aTemCardsVal[i];
			var nForeCardVal = aTemCardsVal[nForeCard];
	...
	fJudge_SAN_SHUN: function(nLen, oCardsNum) {
		...
		var aTemCardsVal = oCardsNum['3'].sort(self.SORTASC);
		for (var i = 1; i < aTemCardsVal.length; i++) {
			var nForeCard = i - 1;
			var nVardVal = aTemCardsVal[i];
			var nForeCardVal = aTemCardsVal[nForeCard];
	...
	fJudge_FEI_JI: function(nLen, oCardsNum) {
		var self = this;
		var bTriple = true;
		var bOther = false;
		if (nLen < 8 || oCardsNum['3'] < 2) {
			return false;
		}
		...
		for (var j = 0; j < aTmpCardsVal.length; j++) {
			...
			if (aPlane.length >= 2) {
				...
				if (nRest == aPlane.length || (nRest == 2 * aPlane.length && oCardsNum['2'].length == aPlane.length)) {
					bOther = true;
				}
				if (bTriple && bOther) {
					break;
	...
	fJudge_SAN_DAI_YI: function(nLen, oCardsNum) {
		var self = this;
		var bTriple = true;
		var bOther = false;
	...
	fJudge_SI_DAI_ER: function(nLen, oCardsNum) {
		var self = this;
		if (oCardsNum['4'].length != 1) {
			return false;
		}
		if (nLen == 6 || (nLen == 8 && oCardsNum['2'].length == 2)) {
			return true;
		}
		return false;
	},
	...
	fTouchCards: function(aSelCards, aMyCards, aSelfUpCards, oLastOut) {
		...
		if (aSelCards.length == 1) {
			...
			if (aSelfUpCards.length == 0) {
				...
				if (plen == 2 && lastPokers.length == 0) {} else if (lastPokers.length != 0) {
